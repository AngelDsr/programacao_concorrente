...
// variaveis para identificar numero de threads lendo e escrevendo
int leit = 0;
int escr = 0;
...
void* tarefa(void* arg) {
   ...
   for(long int i=id; i<QTDE_OPS; i+=nthreads) {
      op = rand() % 100;
      if(op<98) {
         initLeit(&escr, &leit, &mutex, &cond_leit);     

         Member(i%MAX_VALUE, head_p);   /* Ignore return value */

	      saiLeit(&leit, &mutex, &cond_escr);     
	      read++;
      } else if(98<=op && op<99) {
	      initEscr(&escr, &leit, &mutex, &cond_escr);     

         Insert(i%MAX_VALUE, &head_p);  /* Ignore return value */

	      saiEscr(&escr, &mutex, &cond_leit, &cond_escr);     
	      in++;
      } else if(99<=op) {
	      initEscr(&escr, &leit, &mutex, &cond_escr);     
